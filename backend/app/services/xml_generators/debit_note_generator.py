"""Generador de XML para Nota de Debito (08) — DebitNote UBL 2.1."""
from decimal import Decimal
from typing import Dict, Any
from lxml import etree

from .namespaces import DEBIT_NOTE_NS, CAC, CBC, EXT, BASE_NSMAP
...
from ..xml_models.credit_debit_note import NotaDebitoRequest
from ..xml_models.common import Item


def calcular_totales(req: NotaDebitoRequest) -> Dict[str, Any]:
...
def generar_xml_debit_note(req: NotaDebitoRequest,
                            totales: Dict[str, Any]) -> etree._Element:
    nsmap = {None: DEBIT_NOTE_NS, **BASE_NSMAP}
    root  = etree.Element(f"{{{DEBIT_NOTE_NS}}}DebitNote", nsmap=nsmap)
    ...
    # UBL 2.1: PaymentExchangeRate va DESPUÉS de PaymentTerms (más abajo).
    ...
    # UBL 2.1 DebitNote XSD no acepta cac:PaymentTerms (solo cac:PaymentMeans).
    # SUNAT rechazaba con "No se puede leer (parsear) el archivo XML" cuando se incluia.
    # El forma de pago en ND se omite (las NDs no cambian los terminos de pago del documento original).

    # UBL 2.1: PaymentExchangeRate después de AccountingCustomerParty y antes de AllowanceCharge
    if req.tipo_cambio:
        tc = req.tipo_cambio
    ...
    # UBL 2.1 DebitNote usa RequestedMonetaryTotal (no LegalMonetaryTotal como Invoice/CreditNote).
    # SUNAT rechazaba con cvc-particle 2.1 "found LegalMonetaryTotal, but next item should be..."
    lmt = _e(root, f"{{{CAC}}}RequestedMonetaryTotal")
    ...
    for idx, it in enumerate(req.items, 1):
        _agregar_dn_line(root, req, it, idx)

    return root


def _agregar_dn_line(root, req: NotaDebitoRequest, it: Item, idx: int):
    cant = d(it.cantidad)
    ...
    line = _e(root, f"{{{CAC}}}DebitNoteLine")
    _e(line, f"{{{CBC}}}ID", str(idx))
    _e(line, f"{{{CBC}}}DebitedQuantity", fmt_qty(cant),
